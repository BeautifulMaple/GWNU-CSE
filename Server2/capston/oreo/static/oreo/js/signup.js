...
async function registerUserStep1() {
    ...
    if (!userId || !password || !email || !realName) {
        alert('모든 필드를 입력해주세요.');
        return;
    }
    ...
    try {
        const response = await fetch('/register_user/step1/', {
            ...
            body: JSON.stringify({ user_id: userId, password, email, real_name: realName }),
        });

        if (response.ok) {
            const data = await response.json();
            sessionStorage.setItem('user_id', data.user_id); // user_id를 sessionStorage에 저장
...
document.getElementById('id').addEventListener('blur', function () {
    ...
    fetch('/register_user/step1/check_id/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': getCsrfToken(),
